...
const PURCHASE_SUPPLY_PLACEHOLDER = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" role="img" aria-label="Supply placeholder">
    <rect width="96" height="96" rx="18" fill="#f3f0fa" />
    <path d="M25 37 48 25l23 12v27L48 76 25 64Z" fill="#d8d0e8" />
    <path d="m25 37 23 13 23-13M48 50v26" fill="none" stroke="#fff" stroke-width="4" stroke-linejoin="round" />
  </svg>
`.trim())}`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('purchaseHistory').addEventListener('click', async (event) => {
    const id = event.target.dataset.purchaseEdit || event.target.dataset.purchaseDelete;
    if (!id) return;
    const entry = CRMState.purchaseEntries.find((item) => item.id === id);
    if (event.target.dataset.purchaseDelete) {
      if (await showCustomConfirm('ลบรายการซื้อนี้?')) { await deletePurchaseEntry(id); await loadPurchases(); }
      return;
    }
    const e = purchaseElements();
    ...
    e.cost.value = entry.total_cost;
    e.date.value = entry.purchased_at;
    e.supplier.value = entry.supplier || '';
    e.note.value = entry.note || '';
    updatePurchaseUnitCost();
    document.getElementById('purchaseCancelEdit').hidden = false;
  });
  document.getElementById('purchaseCancelEdit').addEventListener('click', resetPurchaseForm);
...
function purchaseUnitLabel(category) { return category === 'stone' ? 'เม็ด' : 'ชิ้น'; }
function getPurchaseCatalog(category) { const cache = CRMState.simulatorCatalogCache || {}; return category === 'stone' ? (cache.stones || STONES) : category === 'charm' ? (cache.charms || []) : (cache.spacers || []); }
function getPurchaseCatalogName(item) { return item?.name?.th || item?.nameTh || item?.name?.en || item?.nameEn || item?.name || item?.id || ''; }
function getPurchaseThumbnail(entry) { const type = purchaseCategory(entry), fallback = type === 'other' ? PURCHASE_SUPPLY_PLACEHOLDER : IMAGE_THUMB_PLACEHOLDER, catalogItemId = entry.catalog_item_id || entry.stone_id; const item = catalogItemId ? getPurchaseCatalog(type).find((candidate) => candidate.id === catalogItemId) : null; const image = item?.image?.primary || item?.image || ''; return { src: image ? withCatalogImageVersion(image, item) : fallback, fallback }; }
function renderPurchaseThumbnail(entry, className = 'purchase-thumb') { const thumbnail = getPurchaseThumbnail(entry); return `<span class="${className}"><img src="${escapeHtml(thumbnail.src)}" alt="" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${thumbnail.fallback}'"></span>`; }
...
async function loadPurchases() {
  ...
  setPurchaseType(e.type.value || 'stone');
  renderPurchases();
}
function renderPurchases() {
  const entries = CRMState.purchaseEntries;
  const total = entries.reduce((sum, entry) => sum + Number(entry.total_cost), 0);
  const qty = entries.reduce((sum, entry) => sum + Number(entry.quantity), 0);
  const month = new Date().toISOString().slice(0, 7);
  document.getElementById('purchaseTotalCost').textContent = formatPurchaseMoney(total);
  document.getElementById('purchaseTotalQty').textContent = `${qty} ชิ้น/เม็ด`;
  document.getElementById('purchaseDistinctItems').textContent = `${new Set(entries.map((entry) => `${purchaseCategory(entry)}:${entry.catalog_item_id || entry.stone_id || purchaseName(entry)}`)).size} ชนิด`;
  document.getElementById('purchaseMonthCost').textContent = formatPurchaseMoney(entries.filter((entry) => entry.purchased_at.startsWith(month)).reduce((sum, entry) => sum + Number(entry.total_cost), 0));
  const range = document.getElementById('purchaseRange').value, category = document.getElementById('purchaseFilterCategory').value, size = document.getElementById('purchaseFilterSize').value, search = document.getElementById('purchaseSearch').value.toLowerCase();
  const filtered = entries.filter((entry) => (category === 'all' || purchaseCategory(entry) === category) && (size === 'all' || (purchaseCategory(entry) === 'stone' && String(entry.size_mm) === size)) && (!search || purchaseName(entry).toLowerCase().includes(search)) && (range === 'all' || (range === 'month' ? entry.purchased_at.startsWith(month) : new Date(entry.purchased_at) >= new Date(Date.now() - 30 * 864e5))));
  document.getElementById('purchaseHistory').innerHTML = filtered.map((entry) => { const type = purchaseCategory(entry), unit = purchaseUnitLabel(type), sizeText = type === 'stone' && entry.size_mm ? ` · ${entry.size_mm}mm` : '', details = [entry.supplier, entry.note].filter(Boolean).map(escapeHtml).join(' · '); return `<article class="purchase-row">${renderPurchaseThumbnail(entry)}<div class="purchase-row-copy"><div><b>${escapeHtml(purchaseName(entry))}</b> <span class="purchase-category-badge">${PURCHASE_CATEGORY_LABELS[type]}</span></div><span>${entry.quantity} ${unit}${sizeText} · ${formatPurchaseMoney(entry.total_cost)} · ${formatPurchaseMoney(entry.unit_cost)}/${unit}</span><small>${entry.purchased_at}${details ? ` · ${details}` : ''}</small></div><div class="purchase-row-actions"><button data-purchase-edit="${entry.id}">แก้ไข</button><button data-purchase-delete="${entry.id}">ลบ</button></div></article>`; }).join('') || '<p>ยังไม่มีรายการซื้อ</p>';
  const itemGroups = {}, categoryGroups = {};
  entries.forEach((entry) => { const type = purchaseCategory(entry), key = `${type}|${purchaseName(entry)}${type === 'stone' ? `|${entry.size_mm}` : ''}`, item = itemGroups[key] || { quantity: 0, cost: 0, type, entry }; item.quantity += Number(entry.quantity); item.cost += Number(entry.total_cost); itemGroups[key] = item; const group = categoryGroups[type] || { quantity: 0, cost: 0 }; group.quantity += Number(entry.quantity); group.cost += Number(entry.total_cost); categoryGroups[type] = group; });
  document.getElementById('purchaseItemSummary').innerHTML = '<h3>สรุปตามรายการ</h3>' + Object.entries(itemGroups).map(([key, group]) => { const [, name, sizeMm] = key.split('|'); const unit = purchaseUnitLabel(group.type); return `<div class="purchase-summary-item">${renderPurchaseThumbnail(group.entry, 'purchase-summary-thumb')}<span>${escapeHtml(name)}${sizeMm ? ` · ${sizeMm}mm` : ''}: ${group.quantity} ${unit} / ${formatPurchaseMoney(group.cost)} / ${formatPurchaseMoney(group.cost / group.quantity)}/${unit}</span></div>`; }).join('');
  document.getElementById('purchaseCategorySummary').innerHTML = '<h3>สรุปตามประเภท</h3>' + Object.entries(PURCHASE_CATEGORY_LABELS).map(([type, label]) => { const group = categoryGroups[type] || { quantity: 0, cost: 0 }; return `<div>${label}: ${group.quantity} ${purchaseUnitLabel(type)} / ${formatPurchaseMoney(group.cost)}</div>`; }).join('');
}
...
function renderOrderCostSummary(order = {}) {
  ...
  const isResolved = cost?.status === 'complete' || cost?.status === 'backfilled';
  const value = (amount) => isResolved && Number.isFinite(Number(amount)) ? formatOrderCostMoney(amount) : 'Unavailable';
  return `<div class="order-cost-summary${isResolved ? '' : ' is-unavailable'}">
    ${isResolved ? (cost?.status === 'backfilled' ? '<div class="order-cost-source">Backfilled estimate</div>' : '') : '<div class="order-cost-unavailable">UNKNOWN / UNRESOLVED COST</div>'}
    ...
    <div><span>Margin</span><strong>${isResolved && Number.isFinite(Number(cost?.marginPercent)) ? `${Number(cost.marginPercent).toFixed(1)}%` : 'Unavailable'}</strong></div>
  </div>`;
}
