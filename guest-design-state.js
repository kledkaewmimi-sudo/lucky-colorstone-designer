// Guest-design persistence is intentionally isolated from normal application state.
...
function normalizeId(value) { const id = typeof value === 'string' ? value.trim() : ''; return id && id.length <= MAX_ID_LENGTH ? id : ''; }
function normalizeUniqueId(value) { const id = Number(value); return Number.isInteger(id) && id > 0 ? id : null; }
function normalizePhysicalStoneSize(value) { const size = Number(value); return PHYSICAL_STONE_SIZES.has(size) ? size : null; }
function normalizeWristSize(value) {
  const size = Number(value);
  return Number.isFinite(size) && size >= 14 && size <= 20 && Math.round(size * 2) === size * 2 ? size : null;
}
function normalizeBeadSize(value) {
  const beadSize = String(value || '').trim();
  return FIXED_BEAD_SIZES.has(beadSize) || beadSize === 'mixed' ? beadSize : '';
}
...
function normalizeCanonicalComponent(component, beadSize) {
  ...
  if (type === 'empty') {
    ...
    return uniqueId === null ? { type } : { type, uniqueId };
  }
  const id = normalizeId(type === 'stone' ? component.id || component.stoneId : type === 'charm' ? component.id || component.charmId : component.id || component.spacerId);
  if (!id) return null;
  const uniqueId = normalizeUniqueId(component.uniqueId);
  const canonical = uniqueId === null ? { type, id } : { type, id, uniqueId };
  if (type !== 'stone') return canonical;
  // Legacy fixed snapshots safely derive the size from their fixed design mode.
  // Mixed snapshots require each stone to carry its own physical size.
  const size = normalizePhysicalStoneSize(component.size ?? component.sizeMm) ?? (FIXED_BEAD_SIZES.has(beadSize) ? Number(beadSize) : null);
  return size === null ? null : { ...canonical, size };
}
...
function normalizeCanonicalDesign(input = {}) {
  ...
  const rawComponents = Array.isArray(input.components) ? input.components : Array.isArray(input.selectedStones) ? input.selectedStones : null;
  if (wristSize === null || !beadSize || !rawComponents || rawComponents.length > MAX_COMPONENTS) return null;
  const mixedPlacingSize = beadSize === 'mixed' ? normalizePhysicalStoneSize(input.mixedPlacingSize) : Number(beadSize);
  if (mixedPlacingSize === null) return null;
  ...
  return { wristSize, beadSize, mixedPlacingSize, selectedCharmIds: normalizedCharmIds, components };
}
...
export function createGuestDesignSnapshot(state = {}, { now = Date.now(), ttlMs = GUEST_DESIGN_SNAPSHOT_TTL_MS } = {}) {
  ...
  return { version: GUEST_DESIGN_SNAPSHOT_VERSION, savedAt, expiresAt: savedAt + ttlMs, step, design };
}
...
export function parseGuestDesignSnapshot(rawSnapshot, { now = Date.now() } = {}) {
  if (typeof rawSnapshot !== 'string' || rawSnapshot.length === 0 || rawSnapshot.length > MAX_SERIALIZED_BYTES) return failure('malformed');
  let parsed;
  try { parsed = JSON.parse(rawSnapshot); } catch { return failure('malformed'); }
  ...
  if (!Number.isFinite(savedAt) || !Number.isFinite(expiresAt) || expiresAt <= savedAt || !ALLOWED_STEPS.has(step) || !design) return failure('invalid');
  if (Number(now) >= expiresAt) return failure('expired');
  return { ok: true, snapshot: { version: GUEST_DESIGN_SNAPSHOT_VERSION, savedAt, expiresAt, step, design } };
}
...
export function reconcileGuestDesignSnapshot(snapshot, catalog = {}) {
  ...
  const validated = createGuestDesignSnapshot({ ...parsed.snapshot.design, currentStep: parsed.snapshot.step }, { now: parsed.snapshot.savedAt, ttlMs: parsed.snapshot.expiresAt - parsed.snapshot.savedAt });
  ...
  return { ok: true, snapshot: { ...validated, design: { ...validated.design, components, selectedCharmIds } }, skipped };
}
