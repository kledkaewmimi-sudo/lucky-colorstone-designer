...
function normalizeTouch(touch) {
  ...
  return {
    utm: {
      source: nullable(touch.utm?.source),
      medium: nullable(touch.utm?.medium),
      campaign: nullable(touch.utm?.campaign),
      content: nullable(touch.utm?.content),
      term: nullable(touch.utm?.term)
    },
    meta: {
      fbclid: nullable(touch.meta?.fbclid),
      fbp: isValidFbp(touch.meta?.fbp) ? touch.meta.fbp : null,
      fbc: isValidFbc(touch.meta?.fbc) ? touch.meta.fbc : null
    },
    referrer: nullable(touch.referrer),
    landingUrl: nullable(touch.landingUrl),
    landedAt: nullable(touch.landedAt)
  };
}
...
export function captureMetaAttribution({ href, referrer = '', cookieString = '', now = Date.now() } = {}) {
  ...
  const existingFbp = readCookie(cookieString, '_fbp');
  const touch = normalizeTouch({
    ...
    meta: {
      fbclid,
      fbp: existingFbp,
      fbc: isValidFbc(existingFbc) ? existingFbc : deriveFbcFromFbclid(fbclid, now)
    },
    referrer,
    landingUrl: url.href,
  ...
  try {
    referrerIsExternal = Boolean(touch.referrer) && new URL(touch.referrer).origin !== url.origin;
  } catch {
    referrerIsExternal = Boolean(touch.referrer);
  }
  const isExternalTouch = Boolean(
    touch.utm.source || touch.utm.medium || touch.utm.campaign || touch.utm.content || touch.utm.term || touch.meta.fbclid || referrerIsExternal
  );
  return { ...touch, isExternalTouch };
}
...
function hydrateMeta(targetTouch, incomingTouch) {
  ...
  return {
    ...targetTouch,
    meta: {
      fbclid: targetTouch.meta.fbclid || incomingTouch.meta.fbclid,
      fbp: targetTouch.meta.fbp || incomingTouch.meta.fbp,
      fbc: targetTouch.meta.fbc || incomingTouch.meta.fbc
    }
  };
}
...
export function updateMetaAttribution(existing, incoming, { updateLastTouch = true } = {}) {
  ...
  const firstTouch = hydrateMeta(state.firstTouch, capture);
  const lastTouch = !state.firstTouch || (updateLastTouch && incoming.isExternalTouch)
    ? capture
    : hydrateMeta(state.lastTouch, capture);
  return { firstTouch, lastTouch };
}
