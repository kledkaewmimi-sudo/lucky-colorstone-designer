...
const orders = [
  { id: '4', date: '2026-01-04', stripePaymentStatus: 'PAID', checkoutSummary: { totalPrice: 40 }, braceletPreviewImage: 'data:image/webp;base64,large', adminOrderNumber: 4 },
  { id: '3', date: '2026-01-03', paymentStatus: 'paid', checkoutSummary: { finalPrice: 30 }, braceletPreviewImage: 'data:image/webp;base64,large', adminOrderNumber: 3 },
  { id: '2', date: '2026-01-02', stripePaymentStatus: 'pending_payment', totalPrice: 20, braceletPreviewImage: 'data:image/webp;base64,large' },
  { id: '1', date: '2026-01-01', paymentStatus: 'paid', netPrice: 10, braceletPreviewImage: 'data:image/webp;base64,large' }
];
...
const page = paginateCrmOrders(orders, 1, 2); assert.equal(page.total, 3); assert.equal(page.totalPages, 2); assert.deepEqual(page.orders.map((order) => order.id), ['4', '3']); assert.equal(page.orders[0].braceletPreviewImage, 'data:image/webp;base64,large'); assert.equal(toCrmOrderSummary(orders[0]).adminOrderNumber, 4);
...
assert.match(serverSource, /braceletPreviewImage:payload->>braceletPreviewImage/);
